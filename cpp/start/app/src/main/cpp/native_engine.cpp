...
void NativeEngine::InitCACert() {
    // Copy the cacert.pem file out of our package into internal storage
    // so cURL can access it with standard file i/o
    const char *filename = "cacert.pem";
    AAssetManager *assetManager = mApp->activity->assetManager;
    AAsset *asset = AAssetManager_open(assetManager, filename, AASSET_MODE_STREAMING);
    bool success = false;

    if (asset != NULL) {
        std::string destPath = mApp->activity->internalDataPath;
        destPath += "/";
        destPath += filename;
        FILE *fp = fopen(destPath.c_str(), "wb");
        if (fp != NULL) {
            const size_t assetSize = AAsset_getLength(asset);
            uint8_t *fileData = new uint8_t[assetSize];
            if (fileData) {
                AAsset_read(asset, fileData, assetSize);
                fwrite(fileData, assetSize, 1, fp);
                delete[] fileData;
                success = true;
            }
            fclose(fp);
        ...
        if (success) {
            HTTPClient::SetCACertPath(destPath);
        }
    }
}

void NativeEngine::InitPackageName() {
    JNIEnv *env = GetJniEnv();
    if (env != NULL) {
        jclass activityClass = env->GetObjectClass(mApp->activity->javaGameActivity);
        jmethodID methodGetPackageName = env->GetMethodID(activityClass, "getPackageName",
                                                          "()Ljava/lang/String;");
        jstring packageNameJavaString = static_cast<jstring>(
                env->CallObjectMethod(mApp->activity->javaGameActivity, methodGetPackageName));
        const char *packageName = env->GetStringUTFChars(packageNameJavaString, NULL);
        if (packageName != NULL) {
            mPackageName = packageName;
        }
        env->DeleteLocalRef(activityClass);
        env->DeleteLocalRef(packageNameJavaString);
    }
}